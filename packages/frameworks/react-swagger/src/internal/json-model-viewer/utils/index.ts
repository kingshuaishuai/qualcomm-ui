...
function _applyValue(
  ...
  path: (string | number)[],
  value: any,
  visitedMapping = new Map<any, any>(),
) {
...
export function createDataType<ValueType = unknown>(
  ...
  if (process.env.NODE_ENV !== "production") {
    console.warn(
      "createDataType is deprecated, please use `defineDataType` instead. See https://viewer.textea.io/migration/migration-v3#use-definedatatype-instead-of-createdatatype for more information.",
    )
  }
  return {
    Component,
...
export async function copyString(value: string) {
  if ("clipboard" in navigator) {
    try {
      ...
      // When navigator.clipboard throws an error, fallback to copy-to-clipboard
      // package
    }
  }
