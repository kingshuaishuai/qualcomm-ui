...
import type {FlatTreeNode, IndexPath, TreeSkipOptions} from "./types.js"
...
export function filter<T>(node: T, options: FilterOptions<T>): T {
  const {create, getChildren, predicate} = options
...
export function map<T, U>(node: T, options: MapOptions<T, U>): U {
  ...
  visit(node, {
    ...
    onLeave: (child, indexPath) => {
      // Add a 0 so we can always slice off the last element to get a unique parent
      // key
...
export function insert<T>(node: T, options: InsertOptions<T>): T {
  const {at, nodes} = options
  if (at.length === 0) {
...
export function replace<T>(node: T, options: ReplaceOptions<T>): T {
...
export function remove<T>(node: T, options: RemoveOptions<T>): T {
...
export function move<T>(node: T, options: MoveOptions<T>): T {
  if (options.indexPaths.length === 0) {
    return node
  }
  for (const indexPath of options.indexPaths) {
...
export function visit<T>(node: T, options: TreeVisitOptions<T>): void {
  const {getChildren, onEnter, onLeave} = options
  const indexPath: IndexPath = []
  const stack: TreeVisitStack<T>[] = [{node}]
...
export type NodeOperation<T> =
  | {index: number; nodes: T[]; type: "insert"}
  | {indexes: number[]; type: "remove"}
  | {type: "replace"}
  | {
      insertIndex: number
      insertNodes: T[]
...
export interface FindOptions<T> extends BaseOptions<T> {
  predicate: (node: T, indexPath: IndexPath) => boolean | undefined
}
 ...
 * @interface
 */
export type CollectionTreeVisitOpts<T> = Omit<
  TreeVisitOptions<T>,
  "getChildren"
> &
  TreeSkipOptions<T>
