...
const fallback: ListCollectionMethods<any> = {
  itemDisabled(item) {
    if (isObject(item) && hasProp(item, "disabled")) {
  ...
  itemLabel(item) {
    ...
    if (isObject(item) && hasProp(item, "label")) {
      return item.label
    }
    return fallback.itemValue(item)
  },
  itemValue(item) {
    if (typeof item === "string") {
      return item
    }
    if (isObject(item) && hasProp(item, "value")) {
      return item.value
    }
    return ""
  },
...
export class ListCollection<T extends CollectionItem = CollectionItem> {
  ...
  constructor(private options: ListCollectionOptions<T>) {
   ...
   */
  getValues = (items: T[] = this.items): string[] => {
  ...
  getItemValue = (item: T | null | undefined): string | null => {
    ...
    return this.options.itemValue?.(item) ?? fallback.itemValue(item)
  }

  /**
   * Whether an item is disabled
   */
  getItemDisabled = (item: T | null): boolean | undefined => {
    if (item == null) {
      return false
    }
    return this.options.itemDisabled?.(item) ?? fallback.itemDisabled(item)
  }
  ...
  stringifyItem = (item: T | null): string | null => {
    ...
    return this.options.itemLabel?.(item) ?? fallback.itemLabel(item)
  }
  ...
  private getByText = (text: string, current: string | null): T | undefined => {
    let items =
      current != null ? wrap(this.items, this.indexOf(current)) : this.items
  ...
  search = (
    ...
    const {currentValue, state, timeout = 350} = options
  ...
  *[Symbol.iterator](): Generator<T, void, unknown> {
   ...
   */
  filter = (
    fn: (itemString: string, index: number, item: T) => boolean,
  ): ListCollection<T> => {
    const filteredItems = this.items.filter((item, index) =>
      fn(this.stringifyItem(item)!, index, item),
    )
  ...
  remove = (...itemsOrValues: Array<T | string>): ListCollection<T> => {
    const values = itemsOrValues.map((itemOrValue) =>
      typeof itemOrValue === "string"
        ? itemOrValue
        : this.getItemValue(itemOrValue),
    )
    return this.copy(
  ...
  moveBefore = (value: string, ...values: string[]): ListCollection<T> => {
    ...
    const indices = values
      .map((value) =>
        this.items.findIndex((item) => this.getItemValue(item) === value),
      )
      .sort((a, b) => a - b)
