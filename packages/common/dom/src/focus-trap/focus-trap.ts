...
export class FocusTrap {
  ...
  private readonly config: FocusTrapOptions
  ...
  constructor(
    ...
    this.doc =
      config.document ||
      getDocument(Array.isArray(elements) ? elements[0] : elements)
    this.config = config
  ...
  private updateTabbableNodes() {
    ...
    // throw if no groups have tabbable nodes and we don't have a fallback focus
    // node either
    if (
  ...
  private addListeners() {
    ...
    this.listenerCleanups.push(
      ...
      addDomEvent(this.doc, "mousedown", this.handlePointerDown, {
        capture: true,
        passive: false,
      }),
  ...
  private handleFocus = (event: FocusEvent) => {
    ...
    if (targetContained || isDocument(target)) {
      ...
      // focus will escape if the MRU node had a positive tab index and user tried
      // to nav forward; it will also escape if the MRU node had a 0 tab index and
      // user tried to nav backward toward a node with a positive tab index
      let nextNode // next node to focus, if we find one
      let navAcrossContainers = true

      if (this.state.mostRecentlyFocusedNode) {
        if (getTabIndex(this.state.mostRecentlyFocusedNode) > 0) {
          // MRU container index must be >=0 otherwise we wouldn't have it as an MRU
          // node...
          const mruContainerIdx = this.findContainerIndex(
            this.state.mostRecentlyFocusedNode,
          )
          // there MAY not be any tabbable nodes in the container if there are at
          // least 2 containers and the MRU node is focusable but not tabbable
          // (focus-trap requires at least 1 container with at least one tabbable
          // node in order to function, so this could be the other container with
          // nothing tabbable in it)
          const {tabbableNodes} = this.state.containerGroups[mruContainerIdx]

          if (tabbableNodes.length > 0) {
            // MRU tab index MAY not be found if the MRU node is focusable but not
            // tabbable
            ...
            if (mruTabIdx >= 0) {
              if (
                ...
                // else, don't wrap within the container as focus should move to
              ...
              // else, don't find in container order without considering direction
              // too
            }
          }
          // else, no tabbable nodes in that container (which means we must have at
          // least one other container with at least one tabbable node in it,
          // otherwise focus-trap would've thrown an error the last time
          // updateTabbableNodes() was run): find next node among all known
          // containers
        } else {
          // check to see if there's at least one tabbable node with a positive tab
          // index inside the trap because focus seems to escape when navigating
          // backward from a tabbable node with tabindex=0 when this is the case
          // (instead of wrapping to the tabbable node with the greatest positive tab
          // index like it should)
          if (
            !this.state.containerGroups.some((g) =>
              g.tabbableNodes.some((n) => getTabIndex(n) > 0),
            )
          ) {
            // no containers with tabbable nodes with positive tab indexes which
            // means the focus escaped for some other reason and we should just
            // execute the fallback to the MRU node or initial focus node, if any
            navAcrossContainers = false
          }
        }
      } else {
        // no MRU node means we're likely in some initial condition when the trap
        // has just been activated and initial focus hasn't been given yet, in which
        // case we should fall through to trying to focus the initial focus node,
        // which is what should happen below at this point in the logic
        navAcrossContainers = false
      }
      ...
      if (nextNode) {
        ...
        this.tryFocus(
          this.state.mostRecentlyFocusedNode || this.getInitialFocusNode(),
        )
      }
    }
  ...
  private handleEscapeKey = (event: KeyboardEvent) => {
    if (
      isEscapeEvent(event) &&
      valueOrHandler(this.config.escapeDeactivates, event) !== false
    ) {
      event.preventDefault()
      this.deactivate()
    }
  ...
  private getInitialFocusNode = () => {
    ...
    if (node === undefined || (node && !isFocusable(node))) {
      ...
      if (this.findContainerIndex(this.doc.activeElement as HTMLElement) >= 0) {
        ...
        // NOTE: `fallbackFocus` option function cannot return `false` (not
        // supported)
        node = firstTabbableNode || this.getNodeForOption("fallbackFocus")
      }
    } else if (node === null) {
      // option is a VALID selector string that doesn't yield a node: use the
      // `fallbackFocus` option instead of the default behavior when the option isn't
      // specified at all
  ...
  activate(activateOptions?: ActivateOptions): this {
    ...
    const checkCanFocusTrap = this.getOption(
      activateOptions,
      "checkCanFocusTrap",
    )
  ...
  deactivate = (deactivateOptions?: DeactivateOptions): this => {
    ...
    const returnFocus = this.getOption(
      options,
      "returnFocus",
    ...
    if (returnFocus && checkCanReturnFocus) {
      ...
      checkCanReturnFocus(returnFocusNode).then(
        finishDeactivation,
        finishDeactivation,
      )
      return this
  ...
  private getNodeForOption = (
    optionName: keyof FocusTrapOptions,
    {
      hasFallback = false,
      params = [],
    }: {hasFallback?: boolean; params?: any[]} = {},
  ) => {
    ...
    if (this.state.tabbableGroups.length > 0) {
      ...
      if (containerIndex < 0) {
        ...
        if (isBackward) {
          ...
          destinationNode =
            this.state.tabbableGroups[this.state.tabbableGroups.length - 1]
              .lastTabbableNode
        } else {
        ...
        if (
          ...
          // last tabbable node, and go to the first tabbable node of the FIRST
          // group
        ...
        if (lastOfGroupIndex >= 0) {
          ...
          const destinationGroupIndex =
            lastOfGroupIndex === this.state.tabbableGroups.length - 1
              ? 0
              : lastOfGroupIndex + 1
...
const isEscapeEvent = (event: KeyboardEvent) =>
  !event.isComposing && event.key === "Escape"
